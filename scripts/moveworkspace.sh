...
for win in `i3-msg -t get_workspaces | sed 's/,/\n/g' | grep name | cut -d\" -f4`; do
    if test $win -lt 10 && !(i3-msg -t get_workspaces | jq ".[] | select(.name==\"$win\")" | grep HDMI-1); then
        i3-msg workspace $win
        i3-msg move workspace to output HDMI-1
    elif test $win -ge 10 && !(i3-msg -t get_workspaces | jq ".[] | select(.name==\"$win\")" | grep eDP1); then
        i3-msg workspace $win
